        ...
        for x in range(0,21):
            ...
            for i in range(nbh):
                ...
                for j in range(nbw):
                    ...
                    DCT_normalized = np.zeros((8,8))
                    
                    #for exact quantization uncomment this
                    #DCT_normalized = np.divide(DCT,QUANTIZATION_MAT).astype(int)   

                    #using approximate divider in quantization
                    
                    for k in range(0,8):
                        for l in range(0,8):
                            DCT_normalized[k][l] = approxDivision(DCT[k][l],QUANTIZATION_MAT[k][l],x)
                            DCT_normalized[k][l] = DCT_normalized[k][l].astype(int)
            ...
            file1 = open("image"+str(x)+"_"+str((ll+1)*25)+".txt","w")
